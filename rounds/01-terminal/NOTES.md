...
| `fractalmem` + 4 переключателя | Строка закона: «память 212 атомов» или «нет связи». Переключатели — в карточке памяти, за кнопкой |
...
### 5.5 Память — одним числом
«Помнит по fractalos: 212 атомов» — это `atoms_total`: он уже собирается, но нигде
не выводится (ANSWERS §4). `offer_count` (0…3) — это «подсказок на ход», и
подписан он именно так. Четыре переключателя спрятаны в карточке памяти.
...
4. **`atoms_total` вывести** в строку закона и карточку памяти.
...
2. **Как называть память в строке закона:** «212 атомов» — честно по данным, но
   это слово из механики. Варианты: «помнит 212», «212 записей».
