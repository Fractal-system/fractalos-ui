...
- **Лента — моноширинным шрифтом**, как сейчас в окне: так выбрал хозяин.
- **Окно по умолчанию одно.** В заголовке кнопки «рядом: Состав · Журнал ·
  Дифф»: нажатая открывает поверхность справа от разговора, повторное нажатие
  или «×» закрывает, «На весь экран» переключает вид целиком. `Ctrl+\` — последняя
...
Все снимки сделаны `tools/shots.mjs` в 1440×900 и 1920×1080; кадры широкого экрана — в 2560×1440, кадр «рядом: Дифф» — в 1920×1080.
...
| 08 | 2560×1440: окно по умолчанию — одно | ![](shots/08-2560-odno-okno-2560x1440.png) | — |
| 09 | 2560×1440: нажата кнопка «рядом: Состав» | ![](shots/09-2560-ryadom-sostav-2560x1440.png) | — |
| 10 | 1920×1080: нажата кнопка «рядом: Дифф» | — | ![](shots/10-ryadom-diff-1920x1080.png) |
...
│   ├── рядом     Состав · Журнал · Дифф   (открывают поверхность справа)
...
└── Вид
    ...
    ├── 3 Журнал     раунд 2
    └── 4 Дифф       раунд 2
Кнопки «рядом» — Состав / Журнал / Дифф справа от разговора.  Палитра (Ctrl+K) — поверх любого вида.

2026-09-25. Строка схемы «любой сегмент → Состав» снята. Щелчок по роли открывает карточку. См. `DECISIONS.md`, поправка этого дня.
```
...
| `Ctrl+P` | проект |
| `Ctrl+\` | открыть или закрыть последнюю поверхность «рядом» |
...
Лента набрана Cascadia Mono 14px — выбор хозяина: «так удобнее». Строка не
длиннее 84 символов, колонка около 1000px по центру, поэтому даже на 2560 строк
на 170 символов не будет. Хром окна — Segoe UI. Пропорциональный шрифт ленты
остался в палитре («Лента: пропорциональный шрифт»), но по умолчанию его нет.

### 5.11 Дополнительные поверхности — кнопками, по умолчанию окно одно
Хозяин: окно на широком мониторе само не делится; есть кнопки для
дополнительных поверхностей, и они делят. В заголовке группа «рядом: Состав ·
Журнал · Дифф» — это переключатели с нажатым состоянием. Открыта одна
поверхность за раз, всегда справа от разговора. У неё своя шапка: имя, «На весь
экран», «×». Вкладки `Ctrl+1…4` по-прежнему показывают вид на всё окно.
...
| **Кнопки «рядом»** | `QButtonGroup(exclusive=false)` из трёх `QToolButton(checkable)` с иконкой; нажатая — QSS `:checked`. Открывают правую панель `QSplitter`. |
| **Шапка боковой поверхности** | `QWidget` 36px: `QLabel` имени, `QToolButton` «На весь экран» и «×». |
...
1. ~~Шрифт ленты~~ — снят: моноширинный (§5.10).
2. ~~Как называть память~~ — снят: хозяин не хочет чисел, память показана словом «работает» / «нет связи».
3. ~~Разделение на 2560~~ — снят: по умолчанию одно окно, делят кнопки «рядом» (§5.11).
