...
Скриншоты текущего окна — `docs/current-shots/` (описание в `README.md` там же).
...
10. **LabCanvas: узел «Учитель» устарел.** Подпись «OpenCode · … · не слот» и текст «TeacherBar + btw (модель OpenCode/OpenRouter)» (`app/labcanvas.py:199-200`) описывают то, чего больше нет: с волны 129 Учитель — правила CPU (CLOSE-HIRE), OpenCode serve погашен. Видно на `docs/current-shots/03-labcanvas-fractalos.png`.
11. **Поясняющие подсказки сегментов Рот/Мозг/Руки не видны.** Сегменту задана подсказка («Кто исполняет объёмную работу…», `app/rail.py:640`), но `apply_availability` ставит каждой кнопке свою — короткое имя провайдера (`app/rail.py:354-356`). При наведении на «Flash» всплывает просто «Flash»; проверено на живом окне.
